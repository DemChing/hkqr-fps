...
class FPS {
    ...
    parse(x) {
        ...
        for (let id in data) {
            ...
            if (id == constant_1.FORMAT_INDICATOR) {
                ...
                let currency = Object.keys(config_1.CURRENCY).filter(v => config_1.CURRENCY[v] == content);
    ...
    generate() {
        ...
        // Transaction Data
        code += this.payload(constant_1.TRANSACTION_CURRENCY, config_1.CURRENCY[this.transactionCurrency]);
     ...
     */
    setStatic() {
        this.initiationPoint = constant_2.STATIC_QR_CODE;
     ...
     */
    setDynamic() {
        this.initiationPoint = constant_2.DYNAMIC_QR_CODE;
        return new event_1.default();
    }
    /**
    ...
    setMerchantAccount(x) {
        ...
        for (let id in x) {
            ...
            if (event.isError()) {
                ...
                if (id == constant_2.MERCHANT_ACCOUNT_UNIQUE)
                    continue;
    ...
    getCurrency(toCode = false) {
        ...
        if (toCode) {
            event.data = config_1.CURRENCY[event.data];
        }
        return event;
    }
    /**
