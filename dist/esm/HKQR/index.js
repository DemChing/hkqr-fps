...
// Import data
import { ISO_COUNTRY, ISO_CURRENCY, ISO_LANGUAGE, ISO_MERCHANT_CATEGORY, MERCHANT, PARTICIPANT } from "./config";
...
export default class HKQR {
    ...
    setMerchantAccount(x) {
        let event = new Event(), uniqueIdentifier;
    ...
    setConvenienceFeeAmount(x, fraction = false) {
        ...
        if (event.isError()) {
            ...
            this.convenienceFeeIndicator = CONVENIENCE_FEE_FIXED;
        }
        return event;
    }
    /**
    ...
    setConvenienceFeePercent(x, fraction = false) {
        ...
        if (event.isError()) {
            ...
            this.convenienceFeeIndicator = CONVENIENCE_FEE_PERCENT;
        }
        return event;
    }
    /**
     ...
     */
    setMerchantInfo(x) {
        let event = new Event(), language, merchantName;
        if (MERCHANT_INFO_LANGUAGE in x) {
            if (x[MERCHANT_INFO_LANGUAGE] in ISO_LANGUAGE) {
                language = x[MERCHANT_INFO_LANGUAGE];
            }
            else {
